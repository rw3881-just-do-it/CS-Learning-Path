...
/*same logic but no conversion
remember how to create a new number!
    reversedhalf = reversedhalf * 10 + x % 10;
    x = x / 10;
inspired by the Better Solution*/
class BetterMySolution{
    public:
    bool isPalindrome(int x) {
        if (x < 0 || (x % 10 == 0 && x != 0)){
            return false;
        ...
        while (x > reversedhalf){
            reversedhalf = reversedhalf * 10 + x % 10;
            x = x / 10; // do it within while scope so actually x will not be changed
        }
